...
: SECRET_SCOPE                  "${SECRET_SCOPE:=releng/gecko/build}"
: LINUX_DEST_DIR                "${LINUX_DEST_DIR:=}"
: MAC_DEST_DIR                  "${MAC_DEST_DIR:=}"
: WINDOWS_DEST_DIR              "${WINDOWS_DEST_DIR:=}"

SSH_KEY=/builds/partner-github-ssh

if [[ "$REPACK_MANIFESTS_URL" == git@* ]]; then
    SECRET_URL="${TASKCLUSTER_PROXY_URL:-http://taskcluster}/secrets/v1/secret/project/${SECRET_SCOPE}/level-${MOZ_SCM_LEVEL}/partner-github-ssh"
