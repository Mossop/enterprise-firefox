...
# cheap and wait on a dependency anyway, so let's make sure that the run as soon
# as possible
REPORT_PRIORITY = "highest"
...
def request(url, method="GET", body=None, raw=False):
    ...
    raise RuntimeError(
        f"{method} {url} failed after {HTTP_ATTEMPTS} attempts: {last_error}"
    )
...
def branch_refs(remote, branch):
    ...
    return [
        ...
        if ref.startswith("refs/heads/") and ref.endswith(f"/{branch}")
    ]


def second_commit(workdir, remote, sha):
    """The parent commit of `sha`.

    A try push is a branch with one commit on top holding what to run, so the
    ...
    while True:
        ...
        for sha, ref in refs:
            if sha not in parents:
                parents[sha] = second_commit(workdir, remote, sha)
            parent = parents[sha]
            if parent != head_rev:
                log(
...
def report_task(
    ...
    try_task_url = f"{task_url}&selectedTaskRun={monitored_id}.0"
    ...
    environment = {
        ...
        "TRY_STATUS_TASK_URL": try_task_url,
    ...
    return {
        ...
        "metadata": {
            "name": name,
            "description": f"Outcome of `{label}` on the try push of this branch ([Treeherder job]({try_task_url}))",
